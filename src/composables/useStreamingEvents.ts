...
export function useStreamingEvents() {
  ...
  async function init() {
    ...
    const conversationIdValue = computed(() => chatStore.activeConversationId) as Ref<string | null>
    ...
    const { listenersReady, cleanup } = useStreaming(conversationIdValue, {
      ...
      onToolResult: (payload) => {
        ...
        chatStore.streaming.buffer = chatStore.streaming.buffer.replace(tag, replacement)
      },
    })
