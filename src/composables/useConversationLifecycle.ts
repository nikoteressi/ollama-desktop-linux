...
export function useConversationLifecycle() {
  ...
  async function createConversation(model?: string, systemPrompt?: string): Promise<string> {
    const finalModel = model?.trim() || ''
    if (!finalModel) throw new Error('A model must be specified to create a conversation')
  ...
  async function updateSystemPrompt(id: string, systemPrompt: string): Promise<void> {
  ...
  return { createConversation, updateTitle, setPinned, updateSystemPrompt, deleteConversation }
}
