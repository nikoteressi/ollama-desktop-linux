...
describe('useConversationLifecycle', () => {
  ...
  it('updateTitle does optimistic update and rolls back on failure', async () => {
    ...
    store.conversations = [{
      id: 'conv-1', model: 'llama3', title: 'Old Title', settings_json: '{}',
      pinned: false, tags: [], draft_json: null, created_at: '', updated_at: ''
    }]
  ...
  it('deleteConversation removes from store and switches active', async () => {
    const store = useChatStore()
    store.conversations = [
      { id: 'conv-1', model: 'llama3', title: 'A', settings_json: '{}', pinned: false, tags: [], draft_json: null, created_at: '', updated_at: '' },
      { id: 'conv-2', model: 'llama3', title: 'B', settings_json: '{}', pinned: false, tags: [], draft_json: null, created_at: '', updated_at: '' },
    ]
