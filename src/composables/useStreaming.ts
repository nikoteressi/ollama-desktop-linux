...
import type { TokenPayload, ThinkingPayload, ThinkingTokenPayload, DonePayload, ToolCallPayload, ToolResultPayload } from '../types/chat'
...
export function useStreaming(
  ...
  async function setupListeners() {
    const unlistenToken = await listen<TokenPayload>('chat:token', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
      if (isThinking.value) {
    ...
    const unlistenThinkingStart = await listen<ThinkingPayload>('chat:thinking-start', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
      isThinking.value = true
      callbacks.onThinkingStart?.(event.payload.conversation_id)
    })

    const unlistenThinkingToken = await listen<ThinkingTokenPayload>('chat:thinking-token', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
      thinkingBuffer.value += event.payload.content
      callbacks.onThinkingToken?.(event.payload)
    })

    const unlistenThinkingEnd = await listen<ThinkingPayload>('chat:thinking-end', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
      isThinking.value = false
      callbacks.onThinkingEnd?.(event.payload.conversation_id, event.payload.duration_ms)
    })
    ...
    const unlistenToolCall = await listen<ToolCallPayload>('chat:tool-call', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
      callbacks.onToolCall?.(event.payload)
    })

    const unlistenToolResult = await listen<ToolResultPayload>('chat:tool-result', (event) => {
      if (conversationId.value && event.payload.conversation_id !== conversationId.value) return
    ...
    unlisteners.push(unlistenToken, unlistenThinkingStart, unlistenThinkingToken, unlistenThinkingEnd, unlistenDone, unlistenToolCall, unlistenToolResult, unlistenError)
  }
  ...
  return { buffer, thinkingBuffer, isThinking, isStreaming, tokensPerSec, reset, cleanup, listenersReady }
}
