...
export function useContextWindow(opts: ContextWindowOptions) {
  ...
  const maxContext = computed(() => {
    const effective = opts.numCtxOverride.value
  ...
  const contextTokens = computed(() => {
    ...
    const hasAssistantReply = chatStore.activeMessages.some(m => m.role === 'assistant')
    const systemPromptTokens = !hasAssistantReply
      ? Math.ceil(opts.globalSystemPrompt.value.length / 4)
      : 0

    return chatStore.totalActiveTokens + inputTokens + imageTokens + systemPromptTokens
  })
