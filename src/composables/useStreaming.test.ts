...
const mockListen = vi.fn().mockImplementation(async (_event: string, _handler: (event: { payload: unknown }) => void) => {
...
vi.mock('@tauri-apps/api/event', () => ({
  listen: (event: string, handler: (event: { payload: unknown }) => void) => mockListen(event, handler),
}))
...
describe('useStreaming', () => {
  ...
  it('onToken callback is NOT called for tokens belonging to a different conversation', async () => {
    ...
    const tokenHandler = mockListen.mock.calls.find(([e]) => e === 'chat:token')?.[1]
    expect(tokenHandler).toBeDefined()
    tokenHandler({ payload: { conversation_id: 'OTHER-conv', content: 'ignored', done: false } })
  ...
  it('onToken callback IS called for tokens matching the active conversation', async () => {
    ...
    const tokenHandler = mockListen.mock.calls.find(([e]) => e === 'chat:token')?.[1]
    tokenHandler({ payload: { conversation_id: 'active-conv-id', content: 'hello', done: false } })

    expect(onToken).toHaveBeenCalledWith({ conversation_id: 'active-conv-id', content: 'hello', done: false })
  })
  ...
  it('onToken callback IS called when conversationId filter is null (accepts all)', async () => {
    ...
    const tokenHandler = mockListen.mock.calls.find(([e]) => e === 'chat:token')?.[1]
    tokenHandler({ payload: { conversation_id: 'any-conv', content: 'hello', done: false } })
