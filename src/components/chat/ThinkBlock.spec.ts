...
function isHidden(wrapper: ReturnType<typeof mount>): boolean {
  return wrapper.find('.think-accordion').classes().includes('think-accordion--closed')
}
...
describe('ThinkBlock', () => {
  ...
  it('isOpen ref is true on mount', () => {
    const wrapper = mount(ThinkBlock, { props: { content: '', isThinking: true } })
    expect((wrapper.vm as unknown as { isOpen: boolean }).isOpen).toBe(true)
  })

  it('isOpen is true while isThinking is true', () => {
    const wrapper = mount(ThinkBlock, { props: { content: 'thoughts', isThinking: true } })
    expect((wrapper.vm as unknown as { isOpen: boolean }).isOpen).toBe(true)
  })
