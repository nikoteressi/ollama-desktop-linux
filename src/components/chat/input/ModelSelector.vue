...
const filteredLibraryModels = computed(() => {
  ...
  const s = modelSearch.value.toLowerCase();
  return modelStore.libraryResults.filter(m => 
...
<template>
  <div class="relative" ref="modelSelectorRef">
    <button
      ...
      :class="isModelDropdownOpen ? 'bg-[var(--bg-active)] ring-1 ring-[var(--accent)]/30' : ''"
    >
      <svg v-if="isActiveModelPulling" class="w-3 h-3 text-[var(--accent)] animate-spin flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
        <path d="M21 12a9 9 0 1 1-6.219-8.56" stroke-linecap="round"/>
      </svg>
      <span class="max-w-[140px] truncate leading-none">{{ activeModelName }}</span>
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="ml-0.5 opacity-50 transition-transform" :class="isModelDropdownOpen ? 'rotate-180' : ''">
    ...
    >
      ...
      <div class="max-h-[380px] overflow-y-auto no-scrollbar">
        ...
        <template v-if="filteredInstalledModels.length > 0">
          <div class="px-3.5 py-2 text-[10px] text-[var(--text-dim)] uppercase tracking-[0.06em] font-[700] bg-[var(--bg-base)]/50 sticky top-0 z-10 backdrop-blur-md">
            Installed Models
          </div>
          <div class="flex flex-col">
            <div
              ...
              :class="m.name === activeModelName ? 'bg-[var(--accent-muted)]/10' : 'hover:bg-[var(--bg-hover)]'"
              @click="selectModel(m.name)"
            >
              <div class="flex items-center justify-between gap-2">
                <div class="flex items-center gap-2 truncate">
                  <span class="truncate text-[13.5px] font-[600]" :class="m.name === activeModelName ? 'text-[var(--accent)]' : 'text-[var(--text)]'">
                    {{ m.name.replace(/-cloud$/, '') }}
                  </span>
                  <ModelTagBadge v-if="m.name.endsWith('-cloud')" tag="cloud" />
                </div>
                <svg v-if="m.name === activeModelName" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="flex-shrink-0">
                  <polyline points="20 6 9 17 4 12"/>
                </svg>
              </div>
              
              <div class="flex flex-wrap items-center gap-1.5 mt-2 transition-opacity group-hover:opacity-100" :class="m.name === activeModelName ? 'opacity-100' : 'opacity-70'">
                 <ModelTagBadge :tag="modelStore.formatBytes(m.size)" />
                 <ModelTagBadge v-if="m.details.parameter_size" :tag="m.details.parameter_size" />
                 <ModelTagBadge v-if="m.details.quantization_level" :tag="m.details.quantization_level" />
                 
                 <template v-if="modelStore.capabilities[m.name]">
                   <ModelTagBadge v-if="modelStore.capabilities[m.name].vision" tag="vision" />
                   <ModelTagBadge v-if="modelStore.capabilities[m.name].tools" tag="tools" />
                   <ModelTagBadge v-if="modelStore.capabilities[m.name].thinking" tag="thinking" />
                 </template>
              </div>
            </div>
          </div>
        ...
        <template v-if="filteredLibraryModels.length > 0">
          <div class="px-3.5 py-2 text-[10px] text-[var(--text-dim)] uppercase tracking-[0.06em] font-[700] bg-[var(--bg-base)]/50 sticky top-0 z-10 backdrop-blur-md flex items-center gap-2">
            <span>From Library</span>
            <div v-if="modelStore.isSearching" class="w-3 h-3 border-2 border-[var(--accent)] border-t-transparent rounded-full animate-spin" />
          </div>
          <div class="flex flex-col">
            <div
            ...
            >
              <div class="flex items-center justify-between gap-1.5 mb-1.5">
                <span class="truncate font-[600] text-[var(--text)] text-[13.5px]">{{ lib.name }}</span>
                <svg class="w-3.5 h-3.5 text-[var(--text-dim)] opacity-40 group-hover:opacity-100 transition-opacity" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
              ...
              <p v-if="lib.description" class="text-[11px] text-[var(--text-muted)] line-clamp-2 leading-relaxed opacity-80">{{ lib.description }}</p>
            </div>
          </div>
        </template>

        <!-- NO RESULTS -->
        <div v-if="modelSearch.trim() && filteredInstalledModels.length === 0 && filteredLibraryModels.length === 0 && !modelStore.isSearching" 
          class="p-8 text-center"
        >
           <p class="text-[13px] text-[var(--text-muted)] mb-4">No models found.</p>
           <button 
        ...
        <div v-if="!modelSearch.trim() && allModelNames.length === 0" class="p-8 text-center text-[12.5px] text-[var(--text-dim)] italic">
