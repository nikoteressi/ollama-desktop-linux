...
function makeCaps(overrides: Partial<ModelCapabilities> & { name: string }): ModelCapabilities {
  return {
    tools: false,
    thinking: false,
...
describe('ChatInput — Web Search toggle visibility', () => {
  beforeEach(() => {
    ...
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === 'get_model_capabilities') return Promise.reject(new Error('mock'))
  ...
  it('GIVEN Cloud=ON + tool-capable model — web search button IS rendered', async () => {
    ...
    modelStore.capabilities['llama3:8b' as ModelName] = makeCaps({ name: 'llama3:8b' as ModelName, tools: true })
...
describe('ChatInput — Thinking toggle visibility', () => {
  ...
  it('GIVEN model="deepseek-r1:7b" (binary-think) — thinking toggle IS rendered', async () => {
    const modelStore = useModelStore()
    modelStore.capabilities['deepseek-r1:7b' as ModelName] = makeCaps({ name: 'deepseek-r1:7b' as ModelName, thinking: true })
...
describe('ChatInput — Submission', () => {
  ...
  it('emits send when handleSubmit is called with text', async () => {
    const wrapper = mountInput()
    const vm = wrapper.vm as unknown as { inputContent: string; handleSubmit: () => Promise<void>; handleFiles: (files: File[]) => Promise<void>; attachments: unknown[] }
...
describe('ChatInput — Attachments', () => {
  ...
  it('handles image files and adds to attachments list', async () => {
    const wrapper = mountInput()
    const vm = wrapper.vm as unknown as { inputContent: string; handleSubmit: () => Promise<void>; handleFiles: (files: File[]) => Promise<void>; attachments: AttachmentMock[] }
