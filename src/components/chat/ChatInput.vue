...
import { useChatStore, base64ToUint8Array, uint8ArrayToBase64 } from '../../stores/chat'
...
const emit = defineEmits<{
  (e: 'send', text: string, images?: Uint8Array[], webSearchEnabled?: boolean, thinkMode?: string, chatOptions?: ChatOptions): void
  (e: 'stop'): void
}>()
...
const linkedContexts = computed(() => chatStore.folderContexts[activeConvId.value ?? ''] ?? [])
...
async function saveSystemPrompt() {
  ...
  await updateSystemPrompt(chatStore.activeConversationId, systemPromptDraft.value)
...
async function selectModel(model: string) {
  ...
  if (chatStore.draftConversation && chatStore.isDraft) {
    ...
    try {
      await tauriApi.updateConversationModel(chatStore.activeConversationId!, model)
    } catch {
      // Ignored
    }
  }
...
watch(activeModelName, async (name) => {
  if (name && name !== 'Select model') {
    await modelStore.fetchCapabilities(name)
  }
}, { immediate: true })

const modelCaps = computed(() => modelStore.getCapabilities(activeModelName.value))

const supportsThinking = computed(() => modelCaps.value?.thinking_toggleable || false)
const supportsThinkingLevels = computed(() => (modelCaps.value?.thinking_levels?.length ?? 0) > 0)
...
const { attachments, isDragging, handleFiles, removeAttachment, clearAttachments, onDragEnter, onDragLeave, onDrop, onPaste } = useAttachments()
...
const { maxContext, contextTokens, isContextNearFull } = useContextWindow({
  ...
  numCtxOverride: computed(() => Number(chatOptions.value?.num_ctx) || undefined),
...
function handleSubmit() {
  ...
  emit(
    ...
    settingsStore.cloud && modelSupportsTools.value ? webSearchEnabled.value : undefined,
    thinkMode,
...
async function handleCompact() {
  if (!activeConvId.value || !activeModelName.value || activeModelName.value === 'Select model') return
...
watch([inputContent, webSearchEnabled, thinkEnabled, thinkLevel, chatOptions], () => {
  debouncedSaveDraft()
}, { deep: true })

// Deep watch for attachments and linkedContexts (debounced)
watch([() => attachments.value.length, () => linkedContexts.value.length], () => {
  debouncedSaveDraft()
})
...
<template>
  ...
  >
    ...
    <div class="rounded-[20px] bg-[var(--bg-user-msg)] border border-[var(--border-strong)] px-[14px] py-[10px] relative">
      <!-- Clipping layer to ensure ContextBar doesn't stick out at the rounded corners -->
      <div class="absolute inset-0 overflow-hidden rounded-[20px] pointer-events-none">
        <ContextBar 
      ...
      <transition name="pop-up">
        <div v-if="isAdvancedOptionsOpen" class="absolute bottom-full right-0 mb-3 z-40">
           <AdvancedChatOptions 
            v-model="chatOptions" 
            @reset="chatOptions = {}" 
           />
      ...
      <!-- Drag overlay -->
      <div v-if="isDragging" class="absolute inset-0 z-30 bg-[var(--accent-muted)] backdrop-blur-[2px] border-2 border-dashed border-[var(--accent)] flex items-center justify-center pointer-events-none rounded-[20px]">
      ...
      <div v-if="linkedContexts.length > 0" class="flex flex-wrap gap-2 mb-2">
        <div v-for="ctx in linkedContexts" :key="ctx.id" class="flex items-center gap-1.5 px-2 py-1 bg-[var(--bg-elevated)] border border-[var(--border-strong)] rounded-lg">
          <svg v-if="ctx.path.includes('.')" class="w-3 h-3 text-[var(--accent)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          <svg v-else class="w-3 h-3 text-[var(--accent)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
          <span class="text-[11px] font-medium text-[var(--text-muted)] truncate max-w-[120px]">{{ ctx.name }}</span>
          <span class="text-[10px] text-[var(--text-dim)]">~{{ ctx.tokens.toLocaleString() }}</span>
          <button @click="removeContext(ctx.id)" class="p-0.5 rounded hover:bg-[var(--bg-active)] text-[var(--text-dim)] hover:text-[var(--text-muted)] cursor-pointer transition-colors">
      ...
      <div class="flex items-center justify-between mt-2">
         <div class="flex items-center gap-2">
            ...
            <button
              v-if="isContextNearFull && !isCurrentChatStreaming && !chatStore.isDraft"
              @click="handleCompact"
            ...
            >
              <svg v-if="isCompacting" class="w-2.5 h-2.5 animate-spin" fill="none" viewBox="0 0 24 24">
         ...
         <div class="flex items-center gap-1.5">
            <button
              ...
              :class="isSystemPromptOpen ? 'bg-[var(--bg-active)] border border-[var(--border-strong)] text-[var(--text)]' : 'bg-[var(--bg-elevated)] border border-[var(--border-strong)] text-[var(--text-muted)] hover:bg-[var(--bg-active)] hover:text-[var(--text)]'"
              title="System prompt"
            ...
            <div class="relative" ref="attachMenuRef">
              <button
                ...
                :class="{ 'text-[var(--text)] bg-[var(--bg-active)]': isAttachMenuOpen }"
                title="Attach"
                aria-label="Attach"
              >
              ...
              <!-- Attachment Menu -->
              <transition name="pop-up">
                <div v-if="isAttachMenuOpen" class="absolute bottom-full left-0 mb-3 z-50 w-44 bg-[var(--bg-surface)] border border-[var(--border-strong)] rounded-xl shadow-2xl py-1 overflow-hidden">
                  <button 
                  ...
                  <button 
                    @click="pickContext(false); isAttachMenuOpen = false"
                    :disabled="isLinking"
                  ...
                  <button 
                    @click="pickContext(true); isAttachMenuOpen = false"
                    :disabled="isLinking"
                    class="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] text-[var(--text)] hover:bg-[var(--bg-hover)] transition-colors cursor-pointer border-none bg-transparent disabled:opacity-50"
                  >
                    <svg class="w-3.5 h-3.5 opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <button
              ...
              class="w-7 h-7 rounded-full flex items-center justify-center transition-colors flex-shrink-0 cursor-pointer"
              :class="webSearchEnabled ? 'bg-[var(--accent-muted)] border border-[var(--accent)] text-[var(--accent)]' : 'bg-[var(--bg-elevated)] border border-[var(--border-strong)] text-[var(--text-muted)] hover:bg-[var(--bg-active)] hover:text-[var(--text)]'"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <button
              ...
              class="w-7 h-7 rounded-full flex items-center justify-center transition-colors flex-shrink-0 cursor-pointer"
              :class="thinkEnabled ? 'bg-[var(--tag-thinking-bg)] border border-[var(--tag-thinking-text)] text-[var(--tag-thinking-text)]' : 'bg-[var(--bg-elevated)] border border-[var(--border-strong)] text-[var(--text-muted)] hover:bg-[var(--bg-active)] hover:text-[var(--text)]'"
            >
            ...
            <button
              ...
              class="w-7 h-7 rounded-full flex items-center justify-center transition-colors flex-shrink-0 cursor-pointer"
              :class="isAdvancedOptionsOpen ? 'bg-[var(--bg-active)] border border-[var(--border-strong)] text-[var(--accent)]' : 'bg-[var(--bg-elevated)] border border-[var(--border-strong)] text-[var(--text-muted)] hover:bg-[var(--bg-active)] hover:text-[var(--text)]'"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <button
              @click="handleSubmit"
              :disabled="!isStreaming && !inputContent.trim() && attachments.length === 0"
              ...
              :class="isStreaming ? 'bg-[var(--bg-user-msg)] hover:bg-[var(--bg-active)]' : (inputContent.trim() || attachments.length > 0) ? 'bg-[var(--text)] hover:bg-[var(--text-muted)]' : 'bg-[var(--bg-user-msg)]'"
            >
              <svg v-if="isStreaming" width="10" height="10" viewBox="0 0 24 24" fill="currentColor" class="text-[var(--text)]">
                <rect x="6" y="6" width="12" height="12" rx="2"/>
              </svg>
              <svg v-else width="13" height="13" viewBox="0 0 24 24" fill="none" :stroke="inputContent.trim() || attachments.length > 0 ? '#1a1a1a' : '#555'" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
