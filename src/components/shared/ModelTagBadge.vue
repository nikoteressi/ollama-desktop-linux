...
const tagConfig = computed(() => {
  ...
  if (t === 'thinking' || t === 'think') return { text: 'Think', cls: 'tag-thinking' }
  if (t === 'cloud') return { text: 'Cloud', cls: 'tag-cloud' }
  if (t === 'embedding' || t === 'embed') return { text: 'Embed', cls: 'tag-embedding' }
  if (t === 'audio') return { text: 'Audio', cls: 'tag-audio' }
  if (/^\d+(\.\d+)?[bB]$/.test(props.tag)) return { text: props.tag.toUpperCase(), cls: 'tag-size' }
