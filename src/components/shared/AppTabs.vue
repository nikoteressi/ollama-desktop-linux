<template>
  <div class="app-tabs sticky top-0 z-40 border-b -mx-6 px-6 py-0 mb-4 select-none overflow-x-auto no-scrollbar">
    <div class="flex items-center gap-0.5">
      ...
      >
        <component v-if="tab.icon" :is="tab.icon" class="w-3.5 h-3.5" style="opacity:0.8" />
        <span>{{ tab.name }}</span>
        ...
        <div
          ...
          style="background: var(--accent); box-shadow: 0 -3px 10px rgba(74,128,208,0.4);"
        />
      </button>
    </div>
  </div>
