<template>
  <div class="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-[13px_14px]">
    <template v-if="authStore.user">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-14 h-14 flex items-center justify-center flex-shrink-0">
            <img src="../../assets/llama-main.png" class="w-full h-full object-contain themed-logo" />
          </div>
          <div class="min-w-0">
            <p class="text-[13.5px] font-semibold text-[var(--text)] truncate">{{ authStore.user.username }}</p>
            <p class="text-[12px] text-[var(--accent)]">{{ authStore.user.email ?? 'nikoteressi@gmail.com' }}</p>
          </div>
        </div>
    ...
    <template v-else-if="showSignInForm">
      <div class="mb-3">
        <p class="text-[13.5px] font-semibold text-[var(--text)] mb-1">Sign in to Ollama</p>
        <p class="text-[12px] text-[var(--text-dim)] leading-relaxed">
          <span v-if="isAwaitingConfirmation">
            Browser opened. Please click <strong>Connect</strong> in your browser to finalize.
            The app will automatically synchronize once confirmed.
          </span>
          <span v-else>
            Connect your Ollama account to securely run cloud models on your native device. This will automatically open your browser to finalize authentication.
          </span>
        </p>
      </div>
      <div class="flex flex-col gap-2.5">
        <p v-if="signInError" class="text-[12px] text-[var(--danger)]">{{ signInError }}</p>
        <div class="flex gap-2 mt-1">
          ...
          >
            ...
            <span v-else-if="isAwaitingConfirmation">Awaiting Connection...</span>
    ...
    <template v-else>
      <div class="flex items-center justify-between">
        <div>
          <p class="text-[13.5px] font-semibold text-[var(--text)] mb-0.5">Ollama account</p>
...
async function submitSignIn() {
  ...
  try {
    ...
    signInError.value = msg || 'Sign in failed. Are you sure Ollama is installed locally?';
  } finally {
