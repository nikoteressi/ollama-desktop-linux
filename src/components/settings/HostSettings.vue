  ...
  <div class="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg overflow-hidden">
    ...
    >
      <div class="flex items-center gap-2.5">
        ...
        <span class="text-[13.5px] text-[var(--text)] font-medium">Ollama Hosts</span>
      </div>
      <svg
        width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
    ...
    <div v-if="hostsExpanded" class="border-t border-[var(--border-subtle)]">
      ...
      <div class="px-3.5 py-2">
        <div v-if="hostStore.hosts.length === 0" class="text-[12px] text-[var(--text-dim)] py-2 text-center">No hosts configured</div>
        <div
        ...
        >
          <div class="flex items-center gap-2.5 min-w-0">
            <span
              ...
              :class="host.last_ping_status === 'online' ? 'bg-[var(--success)]' : host.last_ping_status === 'offline' ? 'bg-[var(--danger)]' : 'bg-[var(--text-dim)]'"
            />
            <div class="min-w-0">
              <p class="text-[13px] text-[var(--text)] font-medium truncate flex items-center gap-1.5">
                {{ host.name }}
                <span v-if="host.is_active" class="text-[10px] text-[var(--accent)]">Active</span>
              </p>
              <p class="text-[11px] text-[var(--text-dim)] font-mono truncate">{{ host.url }}</p>
            </div>
          </div>
      ...
      <div class="border-t border-[var(--border-subtle)] px-3.5 py-3">
        <p class="text-[11px] text-[var(--text-dim)] font-medium uppercase tracking-[0.06em] mb-2">Add New Host</p>
...
function confirmDelete(id: string, isActive: boolean) {
  if (isActive) {
    openModal({
      ...
      message: 'You cannot delete the active host. Please switch to another host first.',
      confirmLabel: 'OK',
      kind: 'info',
      hideCancel: true,
      onConfirm: () => {}
