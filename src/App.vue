<template>
  <div class="h-screen w-screen overflow-hidden flex" style="background: var(--bg-base); color: var(--text); font-family: var(--sans);">

    <!-- 48px Icon strip -->
    <div class="flex-shrink-0 w-12 flex flex-col items-center pt-2 pb-2 gap-1 select-none" style="border-right: 1px solid var(--border); background: var(--bg-base);">
      <!-- Sidebar toggle -->
      <button
      ...
      >
        <svg width="16" height="13" viewBox="0 0 16 13" fill="none">
          <rect x="0" y="0" width="4.5" height="13" rx="1.5" fill="currentColor" opacity="0.35"/>
    ...
    <div
      ...
      :style="{ width: sidebarOpen ? sidebarWidth + 'px' : '0px', background: 'var(--bg-base)', borderRight: sidebarOpen ? '1px solid var(--border)' : 'none' }"
    >
      <!-- Inner container -->
      <div 
...
watch(
  ...
  ([isChat, convId]) => {
    tauriApi.reportActiveView(!!isChat, isChat ? (convId || null) : null).catch(() => {})
  },
  { immediate: true }
)
...
const IconNewChat = markRaw({
  setup() {
    return () => h('svg', { width: 16, height: 16, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': 2.5, 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
...
onMounted(async () => {
  try {
    ...
    if (hostStore.activeHostId) {
      authStore.checkAuthStatus(hostStore.activeHostId).catch((err: unknown) => {
