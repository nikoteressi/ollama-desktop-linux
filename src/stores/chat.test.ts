...
describe('useChatStore', () => {
  ...
  it('loadConversations handles pagination and appends to existing list', async () => {
    const store = useChatStore()
    const page1 = Array.from({ length: 20 }, (_, i) => ({ ...MOCK_CONV, id: `c${i}` }));
    const page2 = [
      { ...MOCK_CONV, id: 'next-page-item' },
    ]
    ...
    expect(mockInvoke).toHaveBeenLastCalledWith('list_conversations', { limit: 20, offset: 20 })
  })
})
