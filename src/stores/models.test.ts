...
const MOCK_MODELS = [
  { name: 'llama3:latest', model: 'llama3:latest', modified_at: '', size: 1000, digest: 'abc', details: {} },
  { name: 'mistral:latest', model: 'mistral:latest', modified_at: '', size: 2000, digest: 'def', details: {} },
]
...
describe('useModelStore', () => {
  ...
  it('deleteModel calls delete_model without any confirm() dialog', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('delete_model', { name: 'llama3:latest' })
  ...
  it('pullModel sets pulling state and calls pull_model', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('pull_model', { name: 'llama3:latest' })
  })

  it('pullModel ignores duplicate pull requests', async () => {
    const store = useModelStore()
    store.pulling['llama3:latest'] = { model: 'llama3:latest', status: 'downloading', percent: 50 }
    mockInvoke.mockResolvedValue(undefined)
