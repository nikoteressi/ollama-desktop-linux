...
export const useHostStore = defineStore('hosts', {
  ...
  getters: {
    activeHost: (state) => state.hosts.find(h => h.id === state.activeHostId) || null,
  },
  actions: {
    async fetchHosts() {
      try {
    ...
    async setActiveHost(id: string) {
      ...
      try {
        ...
        // Clear current models and refetch from new host
    ...
    async initListeners() {
      ...
      listen<HostStatusChangePayload>('host:status-change', (event) => {
        ...
        if (hostIndex !== -1) {
          this.hosts[hostIndex].last_ping_status = payload.status
          this.hosts[hostIndex].last_ping_at = new Date().toISOString()
        }
      })
    }
  }
