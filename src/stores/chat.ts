...
export const useChatStore = defineStore('chat', {
  ...
  getters: {
    ...
    totalActiveTokens: (state) => {
      ...
      if (msgs.length === 0) {
        ...
        return (state.folderContexts[state.activeConversationId] ?? [])
      ...
      if (lastAssistant) {
        ...
        baseTokens = (lastAssistant.prompt_tokens ?? 0) + (lastAssistant.tokens ?? 0)
      } else {
      ...
      const linkedTokens = (state.folderContexts[state.activeConversationId] ?? [])
        .reduce((acc, c) => acc + c.tokens, 0)
  ...
  actions: {
    ...
    async loadConversation(id: string) {
      ...
      try {
        const [rawMessages, dbContexts] = await Promise.all([
          ...
          invoke<FolderContextPayload[]>('get_folder_contexts', { conversationId: id })
        ])
        ...
        const populatedContexts = await Promise.all((dbContexts || []).map(async (ctx) => {
          const payload = await invoke<{ content: string; token_estimate: number }>('link_folder', {
            conversationId: id,
            path: ctx.path,
    ...
    addFolderContext(conversationId: string, context: LinkedContext) {
      ...
      const existing = this.folderContexts[conversationId].find(c => c.path === context.path)
      if (!existing) {
    ...
    removeFolderContext(conversationId: string, contextId: string) {
      if (this.folderContexts[conversationId]) {
        this.folderContexts[conversationId] = this.folderContexts[conversationId]
    ...
    async compactConversation(conversationId: string, model: string, title?: string): Promise<string> {
