...
import type { Model, ModelCapabilities, LibraryModel, LaunchApp } from '../types/models'
...
export const tauriApi = {
  ...
  createConversation: (model: string, title: string | null = null, systemPrompt: string | null = null) =>
  ...
  setActiveHost: (id: string) =>
    invoke<void>('set_active_host', { id }),

  pingHost: (id: string) =>
    invoke<boolean>('ping_host', { id }),
  ...
  logoutHost: (hostId: string) =>
    invoke<void>('logout', { hostId }),
  ...
  checkOllamaSignedIn: () =>
    invoke<boolean>('check_ollama_signed_in'),

  // Service
  startOllama: () =>
    invoke<void>('start_ollama'),

  stopOllama: () =>
    invoke<void>('stop_ollama'),

  ollamaServiceStatus: () =>
    invoke<string>('ollama_service_status'),

  // Folders
  linkFolder: (conversationId: string, path: string) =>
    invoke<{ id: string; path: string; content: string; token_estimate: number }>('link_folder', { conversationId, path }),

  unlinkFolder: (id: string) =>
    invoke<void>('unlink_folder', { id }),
  ...
  getLibraryModelDetails: (slug: string) =>
    invoke<{ readme: string; launch_apps: LaunchApp[] }>('get_library_model_readme', { slug }),
}
