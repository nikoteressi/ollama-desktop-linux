...
use alpaka_desktop_lib::commands::hosts::{add_host, list_hosts, delete_host, ping_host, set_active_host};
use alpaka_desktop_lib::db::hosts::{NewHost, PingStatus};
use rusqlite::Connection;

#[tokio::test]
async fn test_host_crud_integration() {
    let app = mock_builder().plugin(tauri_plugin_notification::init()).build(mock_context(noop_assets())).unwrap();
...
async fn test_from_state_no_active_host_returns_error() {
    ...
    let result = alpaka_desktop_lib::ollama::client::OllamaClient::from_state(
        http_client,
        db_conn,
    )
    .await;

    assert!(result.is_err(), "Expected an error when no active host is configured");
    let err = result.err().expect("Expected Err but got Ok");
    let err_str = err.to_string();
    assert!(
        err_str.contains("No active host") || err_str.contains("not found") || err_str.contains("Not found"),
...
async fn test_from_state_with_inactive_host_returns_error() {
    ...
    direct_conn.execute("UPDATE hosts SET is_active = 0", []).unwrap();
