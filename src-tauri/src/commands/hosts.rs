...
fn validate_host_url(url: &str) -> Result<(), AppError> {
    let parsed = url::Url::parse(url)
        .map_err(|e| AppError::Internal(format!("Invalid host URL: {}", e)))?;
...
mod tests {
    ...
    async fn test_add_host_rejects_file_scheme() {
        let db = setup_test_db();
        let result = core_add_host(db, NewHost {
            name: "Bad".into(),
        ...
        assert!(err_str.contains("not allowed") || err_str.contains("Invalid host URL"), "Error should mention invalid URL: {}", err_str);
    }
    ...
    async fn test_update_host_rejects_non_http_scheme() {
        ...
        let host = core_add_host(db.clone(), NewHost {
            name: "Valid".into(),
        ...
        let result = core_update_host(
            db,
            host.id,
            "Valid".into(),
            "ftp://evil.example.com".into(),
        ).await;
