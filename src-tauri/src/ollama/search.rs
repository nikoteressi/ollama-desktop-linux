...
pub async fn execute_web_search(query: &str, http_client: &reqwest::Client, db: &DbConn) -> Result<String, AppError> {
    log::info!("Executing native Ollama web search for query: {}...", &query.chars().take(8).collect::<String>());
    ...
    let proxy_search_result = if let Ok(active_host) = tokio::task::spawn_blocking(move || {
        let conn = db_clone_proxy.lock().map_err(|_| AppError::Db("Lock poisoned".into()))?;
        let hosts = crate::db::hosts::list_all(&conn)?;
        Ok::<_, AppError>(hosts.into_iter().find(|h| h.is_active))
    }).await {
        if let Ok(Some(h)) = active_host {
            let proxy_url = format!("{}/api/experimental/web_search", h.url.trim_end_matches('/'));
            log::debug!("Attempting web search via host proxy: {}", proxy_url);
            
            let resp = http_client.post(&proxy_url)
            ...
            match resp {
                Ok(r) if r.status().is_success() => {
                    match r.text().await {
                        Ok(text) => {
                            log::info!("Successfully performed web search via host proxy: {}", proxy_url);
                            Some(text)
                        },
                        Err(e) => {
                ...
                Ok(r) => {
                    let status = r.status();
                    log::debug!("Host proxy returned non-success status {}. Falling back.", status);
                    None
                },
                Err(e) => {
                    log::debug!("Network error connecting to host proxy: {}. Falling back.", e);
                    None
                }
            }
        } else {
            log::debug!("No active host found for proxy search. Checking fallback hardcoded localhost.");
            ...
            let resp = http_client.post(local_proxy)
                .header("User-Agent", "OllamaDesktop/0.1.0 (Linux)")
                .json(&json!({ "query": query }))
                .send()
                .await;
    ...
    let active_token = tokio::task::spawn_blocking(move || {
        let conn = db_clone_active.lock().map_err(|_| AppError::Db("Lock poisoned".into()))?;
        let hosts = crate::db::hosts::list_all(&conn)?;
    ...
    let cloud_token = tokio::task::spawn_blocking(|| {
        crate::auth::keyring::get_token("cloud")
    }).await.map_err(|e| AppError::Internal(e.to_string()))??;
    ...
    let hosts_tokens = tokio::task::spawn_blocking(move || {
        let conn = db_clone_all.lock().map_err(|_| AppError::Db("Lock poisoned".into()))?;
        let hosts = crate::db::hosts::list_all(&conn)?;
...
pub async fn perform_search(query: &str, http_client: &reqwest::Client, token: &str, max_results: Option<usize>) -> Result<String, AppError> {
    ...
    if resp.status().is_success() {
        let text = resp.text().await.map_err(|e| AppError::Http(format!("Failed to read search response body: {}", e)))?;
        Ok(text)
    } else {
        let status = resp.status();
        let error_body = resp.text().await.unwrap_or_else(|_| "Could not read error body".to_string());
        Err(AppError::Http(format!("Web search API returned {}: {}", status, error_body)))
    }
}
